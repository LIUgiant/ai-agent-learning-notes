...
## 动手搭建：从一次研究，到连续会话与独立验收

以下短代码是**教学示意**，`Agent`、`validate_data` 等是解释职责的占位接口。建议数据校验门尚未加入当前运行器。

### 版本 1：明确任务可以直接执行

```python
agent = Agent()
result = agent.process_request("计算固定十个城市的两两距离")
```

**先观察**：谁负责列出城市、获取坐标、执行计算？本次是远端托管工具过程，本地收集响应与记录。下一步遇到的问题是：任务并不总有明确条件。

### 版本 2：问题不明确，先澄清

```python
agent = Agent()
first = agent.process_request("帮我分析最近行情")
if is_clarifying_question(first):
    second = agent.process_request("BTC，最近 30 天，分析 MA、RSI、MACD")
```

**为什么复用同一个实例？** 实例保存首轮响应 ID；第二轮带上它，远端才能接着原任务处理。不能因为看到本地 conversation_history，就认为它每轮都被完整发送。

**真实落点**：运行器 `run_backend()` 的 `clarification_agent`；适配器 `_build_request()` 读取 `previous_response_id`，`process_request()` 保存响应 ID。
...
增加这两个对象是为了避免把城市距离上下文带进行情任务；同一澄清任务内部仍复用原对象。

**动手预测**：第二轮改用一个新实例，哪个字段会丢失？答案是旧会话的 `previous_response_id`。先在日志里对照首轮 response ID 和第二轮 request，确认传递关系。

### 版本 4：工具完成以后，增加独立审计

```text
工具执行成功？ → 输入数据来自哪里？ → 能否独立复算？ → 是否回答了需求？
```

这里不是多问模型一句“你确定吗”。距离任务用相同坐标独立重算；行情任务检查实际价格来源。实际记录中，行情获取失败后产生合成价格，因此不能接受为真实行情结论。

**真实落点**：课程 `validate_asean()` 与学习脚本 `audit_remaining.py`。前者的形式检查与后者的独立复核覆盖面不同。

**修改练习**：若城市从 10 个增加到 11 个，除了问题文字，还应检查名单和组合数由 45 变为 55。若增加“先验数据再计算”的强制门，需要改变执行工作流；当前事后审计不会自动阻止远端计算。
